...
func (d *DroneManager) StreamVideo() {
	go func(d *DroneManager) {
		...
		for {
			...
			marshaledJpegBuf, err := json.Marshal(jpegBuf)
			if err != nil {
				log.Println(err)
			}
			...
			if d.isSnapShot {
				...
				ioutil.WriteFile(backupFileName, marshaledJpegBuf, 0644)
				snapshotFileName := snapshotsFolder + "snapshot.jpg"
				ioutil.WriteFile(snapshotFileName, marshaledJpegBuf, 0644)
				d.isSnapShot = false
			}

			d.Stream.UpdateJPEG(marshaledJpegBuf)
		}
	}(d)
}
